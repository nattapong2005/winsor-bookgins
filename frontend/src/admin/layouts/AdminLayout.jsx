...
const AdminLayout = ({ handleLogout }) => {
  ...
  const menuItems = [
    ...
    { path: '/admin/users', label: 'จัดการผู้ใช้งาน', icon: '👥' },
    { path: '/admin/queues', label: 'จัดการคิว', icon: '📅' },
  ];
    ...
    <div className="min-h-screen bg-slate-50 flex font-['Prompt']">
      ...
      >
        <div className="p-4 flex items-center justify-between border-b border-slate-700 h-20">
            {isSidebarOpen ? (
                <span className="text-xl font-bold tracking-tight">VINYL<span className="text-blue-500">ADMIN</span></span>
            ) : (
        ...
        <button 
            ...
            className="absolute top-1/2 -right-3 w-6 h-6 bg-blue-600 rounded-full flex items-center justify-center hover:bg-blue-500 transition-colors shadow-lg"
        >
